 ...
 * @date 2016-4-24
 * @author hukai
 * @todo 這是一個練習示例，實際中意義不大
 */
public class RedisUtilTest {
	/**
	 * JedisPool是一个线程安全的网络连接池。可以用JedisPool创建一些可靠Jedis实例
	 * 可以从池中拿到Jedis的实例。这种方式可以解决那些问题并且会实现高效的性能.
	 **/
	...
	public RedisUtilTest(JedisPoolConfig config, String ip, int prot) {
		if (pool == null) {
	 ...
	 *            连接池对象
	 */
	public RedisUtilTest(JedisPool pool) {
		if (this.pool == null) {
			this.pool = pool;
		}
	}
	 ...
	 * @param @param key
	 * @param @param member
	 * @param @return todo：有序集合key中，成为key为member的排名
	 */
	 ...
	 * @param @param max
	 * @param @param min
	 * @param @return todo：返回有序集合key中，score位于[min,max]之间成员的key
	 */
	 ...
	 * @param @param key
	 * @param @param member
	 * @param @return todo：返回键为key的有序集合中，成员key为member的score
	 */
	 ...
	 * @param @param start
	 * @param @param end
	 * @param @return todo：根据排名删除指定区间的成员
	 */
	 ...
	 * @param @param start
	 * @param @param end
	 * @param @return todo：根据分数删除指定区间内的元素，以0基础索引
	 */
	...
	/**
	 * @date：2016-4-24
	 * @author：hukai todo：返回满足pattern表达式的所有key
	 */
